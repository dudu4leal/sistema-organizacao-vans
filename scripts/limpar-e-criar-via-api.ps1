# ============================================================
# SCRIPT: Limpar torcedores problematicos e recriar via API
# USO: 1. Pare e reinicie a aplicacao (dotnet run)
...
Write-Host " 1. LIMPANDO TORCEDORES ANTIGOS" -ForegroundColor Cyan
Write-Host "==============================" -ForegroundColor Cyan

# Busca todos os usuarios atuais
$usuarios = Invoke-RestMethod -Uri "$apiBase/usuarios" -Method GET

# Lista de nomes que queremos (para identificar os que foram criados via SQL)
$nomesAlvo = @(
...
foreach ($u in $usuarios) {
    if ($nomesAlvo -contains $u.nome) {
        try {
            Invoke-RestMethod -Uri "$apiBase/usuarios/$($u.id)" -Method DELETE
            Write-Host "  Removido: $($u.nome)" -ForegroundColor Yellow
            $removidos++
        } catch {
            Write-Host "  Erro ao remover $($u.nome): $_" -ForegroundColor Red
        }
    }
}
Write-Host "Removidos: $removidos torcedores" -ForegroundColor Green

Write-Host "`n==============================" -ForegroundColor Cyan
Write-Host " 2. OBTENDO ROTAS" -ForegroundColor Cyan
Write-Host "==============================" -ForegroundColor Cyan
...
Write-Host "`n==============================" -ForegroundColor Cyan
Write-Host " 3. CRIANDO TORCEDORES" -ForegroundColor Cyan
Write-Host "==============================" -ForegroundColor Cyan
...
# --- BINGEN (20 pessoas: 10 originais + 10 vindas de Cascatinha/Centro) ---
Write-Host "`n--- VAN BINGEN (20 pessoas) ---" -ForegroundColor Cyan
$bingen = @(
    ...
    # Redistribuidos de Cascatinha/Centro
...
# --- QUITANDINHA (24 pessoas: 14 originais + 10 vindas de Cascatinha/Centro) ---
Write-Host "`n--- VAN QUITANDINHA (24 pessoas) ---" -ForegroundColor Cyan
$quitandinha = @(
